...
int amMain( SceSize arglen, void *argp )
{
	...
	while( st_running ){
		...
		/* 指定されたボタンのフラグのみ取り出す */
		pad.Buttons &= (~( ~0 ^ AM_TOGGLE_BUTTONS ));
		
		if( pad.Buttons == AM_TOGGLE_BUTTONS ){
...
{
	st_main_thid = sceKernelCreateThread( "AutoMute Main", amMain, 32, 0x900, PSP_THREAD_ATTR_NO_FILLSTACK, 0 );
	st_disp_thid = sceKernelCreateThread( "AutoMute Notice", noticeThread, 16, 0x900, PSP_THREAD_ATTR_NO_FILLSTACK, 0 );
